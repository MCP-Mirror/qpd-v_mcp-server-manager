...
var MCPServersProvider = class {
  ...
  constructor() {
    ...
    this.addKnownConfigPath(path.join((0, import_os.homedir)(), "Library/Application Support/Claude/claude_desktop_config.json"));
  }
  refresh() {
  ...
  async searchForConfigs() {
    ...
    for (const searchPath of searchPaths) {
      try {
        const files = await (0, import_glob.glob)("**/*config.json", {
          cwd: searchPath,
          absolute: true,
...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode2.commands.registerCommand("mcp-guide.searchConfigs", async () => {
      const searching = await vscode2.window.withProgress({
      ...
      vscode2.window.showInformationMessage("MCP config file search completed.");
    }),
    vscode2.commands.registerCommand("mcp-guide.addServer", async () => {
      ...
      if (!configFiles || configFiles.length === 0) {
        vscode2.window.showErrorMessage("No MCP config files found. Please search for config files first.");
        return;
      }
      const selectedConfig = await vscode2.window.showQuickPick(
      ...
      try {
        ...
        const content = await fs2.readFile(selectedConfig.configPath, "utf-8");
        const config = JSON.parse(content);
        config.mcpServers = config.mcpServers || {};
        config.mcpServers[serverName] = serverConfig;
        await fs2.writeFile(selectedConfig.configPath, JSON.stringify(config, null, 2));
        mcpServersProvider.refresh();
        vscode2.window.showInformationMessage(`Server ${serverName} added successfully.`);
      } catch (error) {
        vscode2.window.showErrorMessage(`Failed to add server: ${error}`);
      }
    }),
  ...
  vscode2.commands.executeCommand("mcp-guide.searchConfigs");
}
function deactivate() {
}
