          ...
          for (const file of entries) {
            ...
            try {
              ...
              const isMCPConfig = (
                ...
                config.mcpServers && typeof config.mcpServers === "object" || // Contains MCP server commands
                (content.includes("@modelcontextprotocol/server") || content.includes("mcp-server")) || // File path contains MCP-related terms
                file.toLowerCase().includes("mcp") || file.toLowerCase().includes("claude") || file.toLowerCase().includes("cline")
              );
              if (isMCPConfig) {
...
function activate(context) {
  ...
  context.subscriptions.push(
    ...
    vscode2.commands.registerCommand("mcp-guide.searchConfigs", async () => {
      ...
      if (results.length === 0) {
        vscode2.window.showInformationMessage("No MCP config files found.");
        return;
      }
      ...
      quickPick.onDidAccept(async () => {
        ...
        if (selectedPaths.length > 0) {
          selectedPaths.forEach((path2) => {
            mcpServersProvider.addKnownConfigPath(path2);
          });
          vscode2.window.showInformationMessage(`Added ${selectedPaths.length} config file(s).`);
        }
      });
      quickPick.show();
    }),
